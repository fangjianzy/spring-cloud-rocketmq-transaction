package com.fangjian.service.order.mqservice;
...
import com.fangjian.service.order.config.Jms;
...
class TransactionListenerImpl implements TransactionListener {
    ...
    public LocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        log.info("=========本地事务开始执行=============，业务场景是本地下订单，产品服务减少库存");
        ...
        log.info("本地进行保存订单，但先需要确定库存有库存");
        int result = produceOrderService.save(userId, productId, total);
        //模拟执行本地事务end========
        //TODO 实际开发下面不需要我们手动返回，而是根据本地事务执行结果自动返回
        //1、二次确认消息，然后消费者可以消费
        if (result == 0) {
        	log.info("本地事务成功，发送MQ信息COMMIT_MESSAGE，订阅系统才能看到这个key");
            //return LocalTransactionState.COMMIT_MESSAGE;
        	return LocalTransactionState.COMMIT_MESSAGE;
        }
        //2、回滚消息，Broker端会删除半消息
        if (result == 1) {
        	log.info("本地事务成功，发送MQ信息ROLLBACK_MESSAGE，订阅系统看不到此信息，并且MQ3天后自动删除此信息");
            return LocalTransactionState.ROLLBACK_MESSAGE;
        }
        //3、Broker端会进行回查消息
        if (result == 2) {
        	log.info("通知MQ本地事务执行情况为UNKNOW");
    ...
    public LocalTransactionState checkLocalTransaction(MessageExt msg) {
         ...
         */
        log.info("本地要根据这个key去查是否本地执行成功，如果成功则返回COMMIT_MESSAGE，否则返回rollback");
